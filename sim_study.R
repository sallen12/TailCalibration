...
for (del_i in 1:del_n) {
  ...
  del_plots[[del_i]] <-
    plot_calibration(df, type = "Marginal tail calibration", 0, 
                     title = paste0("Min: ", round(del_t[1], 2), ", Max: ", round(del_t[2], 2)), ylims = c(-0.5, 2.1))
}
...
plot_calibration(df, type = "Probabilistic tail calibration", 1/12)
...
plot_calibration(df, type = "Probabilistic tail calibration", 0)
