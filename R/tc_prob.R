...
#'
#'
#' @seealso \code{\link{tail_cal}} \code{\link{tc_marg}}
#'
#'
#' @section References:
...
#' F_x <- function(x, mu1, mu2, loi) plogis(x, mu1)*loi + pnorm(x, mu2)*(1 - loi)
#'
#' mu1 <- rnorm(mu + 1, sd = 0.5)
#' mu2 <- mu
#' loi <- mu > 1
#'
#' cpit <- tc_prob(y, F_x, t = c(-2, 0, 2), mu1 = mu1, mu2 = mu2, loi = loi)
#'
#'
#' @name tc_prob
NULL
